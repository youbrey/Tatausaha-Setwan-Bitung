...
DocumentGenerator = Callable[[str], list[Path]]
...
class _LivePreviewWorker(QThread):
    ...
    def run(self) -> None:
        try:
            documents = [Path(path) for path in self.generator(self.temporary_directory)]
            if not documents:
...
class LiveDocumentPreview(QFrame):
    ...
    def schedule(self, generator: DocumentGenerator) -> None:
        """Jadwalkan update terbaru dan batalkan hasil request yang sudah usang."""
    ...
    def showEvent(self, event) -> None:
        ...
        self._active = True
        if self._last_generator is not None:
    ...
    def _start_update(self) -> None:
        if not self._active or self._pending_generator is None:
            return
    ...
    def _preview_ready(
        ...
        self._close_pdf()
        self._pdf_document = fitz.open(pdf_path)
        self._page_index = 0
        self._zoom = 1.0

        blocker = QSignalBlocker(self.document_selector)
        self.document_selector.clear()
        self.document_selector.addItems([path.name for path in self._documents])
        self.document_selector.setCurrentText(self._source_path.name)
        del blocker
        self.document_selector.setEnabled(len(self._documents) > 0)
    ...
    def _change_document(self, name: str) -> None:
        if not name or self._last_generator is None:
            return
        self._pending_generator = self._last_generator
        self._request_id += 1
        if self._active:
    ...
    def _render_page(self) -> None:
        ...
        pixmap = QPixmap()
        pixmap.loadFromData(rendered.tobytes("png"))
        self.canvas.setText("")
